...
def Generator(n_samples, noise=None):
    ...
    output = spin.gan.tflib.ops.deconv2d.Deconv2D(
        "Generator.2", 4 * DIM, 2 * DIM, 5, output
    )
    if MODE == "wgan":
        output = spin.gan.tflib.ops.batchnorm.Batchnorm(
            "Generator.BN2", [0, 2, 3], output
        )
    output = tf.nn.relu(output)

    # [n_samples, 4, 4, 2*DIM] -> [n_samples, 8, 8. DIM]
    output = spin.gan.tflib.ops.deconv2d.Deconv2D(
        "Generator.3", 2 * DIM, DIM, 5, output
    )
    if MODE == "wgan":
        output = spin.gan.tflib.ops.batchnorm.Batchnorm(
            "Generator.BN3", [0, 2, 3], output
        )
    output = tf.nn.relu(output)
...
def Discriminator(inputs):
    ...
    output = spin.gan.tflib.ops.conv2d.Conv2D(
        "Discriminator.1", 1, DIM, 4, output, stride=2
    )
    output = LeakyReLU(output)

    # [n_samples, 8, 8, DIM] -> [n_samples, 4, 4, 2*DIM]
    output = spin.gan.tflib.ops.conv2d.Conv2D(
        "Discriminator.2", DIM, 2 * DIM, 4, output, stride=2
    )
    if MODE == "wgan":
        output = spin.gan.tflib.ops.batchnorm.Batchnorm(
    ...
    output = spin.gan.tflib.ops.linear.Linear(
        "Discriminator.Output", 2 * 2 * 4 * DIM, 1, output
    )
...
if MODE == "wgan":
    ...
    disc_train_op = tf.train.AdamOptimizer(learning_rate=2e-4, beta1=0.5).minimize(
        disc_cost, var_list=disc_params
    )
    ...
    for iteration in xrange(ITERS):
        ...
        for i in xrange(disc_iters):
            ...
            _disc_cost, _ = session.run(
                [disc_cost, disc_train_op], feed_dict={real_data: _data}
            )
